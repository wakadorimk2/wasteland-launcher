...
`detectConflicts` は `src/core/conflicts.ts` の Phase 3 実装である。現在は replay effect overlap を優先し、fallback が必要な operation に footprint overlap と normalized XPath grouping を適用する。最後の load-order operation を winner として返す `ConflictGroup` shape は維持している。
...
React UI は `ContextPack.conflicts` を優先して conflict row を作り、`ContextPack.trace` を operation evidence として join する。exact/fallback chip、winner、operation timeline、evidence pane は React 側 derived model で生成する。append-only の共存的な変更は review 対象から抑制する。

`trace: []` または trace budget により replay evidence がない context では、UI evidence は `not traced` 相当になる。この場合も `ContextPack.conflicts` は表示できるが、exact replay proof や before/after の説明はできない。
...
進捗: 反映途中。`detectConflicts` は replay effect overlap を exact group として作り、fallback が必要な operation に footprint と normalized XPath を適用する。React UI は `ContextPack.conflicts` を主入力として Phase 3 viewer を表示している。
...
- `append` 同士の同一 parent slot は原則として共存するため、単独では review 対象にしない。
...
- React UI は `ui/src/model.ts` で `ContextPack.conflicts` を `ContextPack.trace` と join し、exact/fallback chip、winner、operation timeline、evidence pane を構築する。
...
- Phase 3 React conflict viewer。
  - `ContextPack.conflicts` 優先。
  - exact/fallback chip。
  - winner / operation timeline / evidence pane。
  - append coexisting changes の review 抑制。
...
- append-only coexistence の抑制が false negative にならないよう、remove / anchored insert / scalar collision / replay diagnostic の例外を fixture で増やす。
