...
`detectConflicts` は `src/core/conflicts.ts` の Phase 3 実装である。現在は replay effect overlap を `targetKey` で優先し、fallback が必要な operation に footprint overlap と normalized XPath grouping を適用する。`ContextPack.diagnosticGroups` が主出力で、`conflicts` は移行用 alias として残している。
...
React UI は `ContextPack.diagnosticGroups` を優先して conflict row を作り、`ContextPack.trace` を operation evidence として join する。target key、classification、confidence、operation timeline、evidence pane は React 側 derived model で生成する。same-parent append / insert は sibling order dependency として低めの risk で残す。
